...
- **Схема:** Mini App и API на одном домене за Caddy с TLS от Let's Encrypt. Наружу открыты только
  80/443 и SSH; PostgreSQL, Redis и API без публичных портов. Порт Portainer — см. риск R1.
...
1. **HostKey → переустановка:** указать публичный ключ `id_ed25519.pub`, удалить старый ключ,
   сгенерировать новый root-пароль, Ubuntu 22.04 (+ Portainer, если он нужен — см. R1).
2. **Portainer** (если установлен): сразу после установки задать пароль администратора.
...
- **R1. Portainer наружу.** Открытый 9443 до установки пароля администратора позволяет перехватить
  панель. Предложение: не открывать 9443 в ufw, а ходить через SSH-туннель; либо ограничить
  доступ IP владельца. Решение за владельцем.
...
- **R4. Первый вход.** Одновременный первый вход двумя запросами может дать `IntegrityError` на
  `users.telegram_user_id` (записано в `dev/ProjectLog.md`); последовательная загрузка из #11 снижает
  риск со стороны Mini App, но не закрывает его на сервере.
