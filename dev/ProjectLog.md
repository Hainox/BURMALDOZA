...
- Каждый issue, handoff, запуск и отчёт должен указывать точные модель и effort. Workflow задаёт маршруты Codex `gpt-6-sol/high` и `gpt-6-astra/xhigh`, Claude Code с явно выбранной в `/model` моделью и `--effort`, CCode `meta/muse-spark-1.3-contributor/high`. Для ранее запущенных Claude-сессий точная модель и effort не были записаны; их не выводим задним числом.
- На дату этой записи Claude Code подготовил серверные изменения PR #10; Codex завершил исправления аудита и открыл stacked PR #12. CCode #6 остаётся за визуальной Blackjack-комнатой после PR #8; issue #11 ждёт завершения #6 и серверного изменения, чтобы не делить `+page.svelte`.
