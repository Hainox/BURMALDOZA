...
## 2026-09-24 — CCode issue #1: Slot stop continuity followup (meta/muse-spark-1.3-contributor, high)

- Новая ветка `ccode/slot-stop-continuity-followup` от свежего `origin/main` (`80ef473`); локальная `ccode/slot-stop-continuity` и замещённый коммит `7146319` не использовались.
- Issue #2 — дубликат #1: его требования (2–3 с вертикального движения, 0/110/220 мс, непрерывность, порядок left→right без snap, финальная сетка = сервер, reduced motion, regression + check/test/build/e2e, отчёт о причине/рисках) полностью покрыты критериями #1; отдельный scope из #2 отсутствует.
- Измерение на текущем main (Chromium, Windows, Playwright probe): computed transform движется в travel (`matrix(...,-0.007)` → `matrix(...,-3.78)` за 160 мс); launch 0/110/220 мс; stop-start 2000/2160/2320 мс; `data-spin-duration=2940`; landed-метки 2135/2317/2446 мс от старта спина (разброс 311 мс, порядок left→center→right); фазы `spinning → stopping-left → stopping-center → stopping-right → settled` без одновременного snap всех барабанов. Runtime-дефект «все три останавливаются резко/одновременно» не воспроизвёлся — анимация не переписывалась.
- Проверка в `apps/miniapp/tests/e2e/slot-stop.spec.ts`: новый тест измеряет computed transform во время travel, launch 0/110/220, `data-spin-duration` в окне 2000–3000, отдельные landed-метки трёх барабанов (монотонность + разброс ≥100 мс), `data-stopped-reels=3`, 3×`landed`, `slot-confirmed-grid` и совпадение итоговой сетки с сервером. Одни CSS-классы не используются. Существующие тесты фаз/порядка/settlement/reconnect/reduced motion оставлены.
- Проверки: `pnpm miniapp:check` — 0/0; `pnpm miniapp:test` — 25 passed; `pnpm miniapp:build` — passed; `pnpm --dir apps/miniapp exec playwright test tests/e2e/slot-stop.spec.ts tests/e2e/accessibility.spec.ts` — 9 passed. Браузер: Chromium на Windows. Ручная проверка iOS/Android WebView и Telegram WebView не выполнялась — ограничение отчёта; нужен проход на 320/390/768 и reduced-motion в реальном WebView.
- Открыто: PR из `ccode/slot-stop-continuity-followup` в `main`; после слияния закрыть #1 и #2 (с пометкой о дубликате). `+page.svelte`, API, RNG, ledger, Blackjack и Hold’em не менялись.
