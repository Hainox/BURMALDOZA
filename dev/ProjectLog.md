...
- Проверка локальной страницы Playwright: по 52 уникальные карты в каждом наборе, 51 реальный портрет и placeholder PTRS в B, загрузка изображений, переключение и фильтры; ширины 320/390/768 без горизонтального переполнения и ошибок консоли. `git diff --check` чистый. GitHub CI и Pages-деплой этой версии проверяются после checkpoint commit.
