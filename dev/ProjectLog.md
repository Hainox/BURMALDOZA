...
- Проверка: локальный Playwright verifier подтвердил состав, декодирование портретов и viewport 390×844; GitHub Verify для PR #8 ранее прошёл. После этого мобильная README-страница и новые файлы требуют повторного CI.
- Открыто: проверить новый diff/CI и дать CCode ссылку на опубликованные исходники после обновления PR; PR #8 остаётся отдельным checkpoint до слияния.
