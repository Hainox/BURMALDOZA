...
- Deck B включает 51 портрет из IOP Wiki и нейтральный placeholder PTRS, так как его портретный источник не подтверждён. Исходники и атрибуция описаны в manifest; согласие владельца ограничено этим некоммерческим превью и не называется общей лицензией.
...
- Проверка Playwright на локальной и опубликованной странице: по 52 уникальные карты в каждом наборе, 51 реальный портрет и placeholder PTRS в B, 52 изображения карт декодируются, фильтр находит PTRS; ширины 320/390/768 без горизонтального переполнения, ошибок консоли и неуспешных запросов. Room concept загрузился. `git diff --check` чистый. GitHub PR CI run 35878105317 прошёл: Python lint/tests, Mini App check/unit/build/E2E, Docker Compose и Monte Carlo. Pages run 35878547294 прошёл; live URL вернул HTTP 200.
- Открыто: подтвердить источник PTRS; PR #8 остаётся draft до отдельного решения о слиянии. CCode runtime UI не запускать до выполнения gate в issue #6.
