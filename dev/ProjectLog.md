...
- Проверка: `node --check`, `pnpm miniapp:check` (0 warnings), `pnpm miniapp:build` и `git diff --check` прошли. Playwright Chromium проверил локальный Pages build: 52 уникальные карты (13 каждой масти), фильтры (червы — 13, J/Q/K — 12), поиск UMP45, карточное досье, декодирование всех портретов, загрузку комнаты/рубашки и ширины 320–1440 px без горизонтальной прокрутки или ошибок браузера. Публичный Pages workflow и live URL проверяются после отправки ветки.
