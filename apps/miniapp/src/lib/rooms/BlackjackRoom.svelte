...
<div class="game-room blackjack-room">
  ...
  <section class="felt-table" class:dealing={motion === 'resolving'} aria-label="Blackjack table">
    ...
    <div class="hand dealer-hand"><span class="hand-label">DEALER <b>?</b></span><div class="cards">{#each dealerCards as card, index}<div class:hole-card={index === 1} class="playing-card" style={`--card-delay: ${index * 130}ms; --card-rotation: ${index === 1 ? '5deg' : '-4deg'}; --card-offset: ${index === 1 ? '3px' : '0px'}`}>{card}</div>{/each}</div><span class="total">{motion === 'outcome' || motion === 'settle' ? '17' : '—'}</span></div>
